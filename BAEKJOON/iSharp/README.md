...
i#은 기본 변수형과 배열([]), 참조(&), 포인터(*)를 제공한다. 배열, 참조, 포인터는 순서에 상관없이 혼합해서 사용할 수 있다. 즉, `int`의 참조의 참조의 배열의 포인터도 올바른 타입이다. `int&&[]*`
...
a의 타입은 `int&&[]*`, b는 `int&`, c는 `int&*`이 된다. 변수의 오른편에 있는 변수형은 순서를 뒤집어서 왼편에 붙일 수 있다. 따라서, `int*& a`는 `int a&*`와 같다.
