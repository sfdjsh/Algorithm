...
```
위와 같은 식으로 방문 체크 리스트를 만들고 반복문을 돌아 전파 길이만큼 방문 체크를 해주고, 
시작점이 n보다 크다면 종료하는 식으로 답을 리턴하였지만, 
효율성 테스트에서 실패가 나왔다.

보완)
stations를 기준으로 반복문을 돌아,
stations에서 전파되지 않은 아파트 구역을 내장함수인 ceil을 이용해서 전파크기만큼 나눠,
답을 리턴하였다.
```
