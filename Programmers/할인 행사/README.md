...
def solution(want, number, discount):
    ...
    for d in range(len(discount) - 9):
        dis_lst = discount[d:d + 10]        # 인덱스부터 10일 연속 할인하는 제품 리스트 

        tmp = True
        for i in range(len(want)):
            cnt = dis_lst.count(want[i])    # 원하는 제품의 할인하는 수량 파악 
            if cnt != number[i]:            # 원하는 제품의 수량과 할인하는 제품 수량이 일치하지 않을 때, break
                tmp = False                 
                break                       
                
        if tmp == True:                     # 각각의 원하는 제품 수량이 일치한 경우    
            answer += 1
